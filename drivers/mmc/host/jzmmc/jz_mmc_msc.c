...
#ifdef CONFIG_SOC_JZ4770       //4770 can use 48M clock                      
	if (rate > SD_CLOCK_FAST) {
		cpm_set_clock(cgu_clk, 48 * 1000 * 1000);
		clkrt = msc_calc_clkrt(0, rate);
		REG_MSC_LPM(host->pdev_id) |= 0x1 << 31;	// send cmd and data at clock rising
	} else {
		cpm_set_clock(cgu_clk, 24 * 1000 * 1000);
		clkrt = msc_calc_clkrt(1, rate);
		REG_MSC_LPM(host->pdev_id) &= ~(0x1 << 31);	// send cmd and data at clock falling
	}
#else
	if (rate > SD_CLOCK_FAST) {
		rate = SD_CLOCK_FAST;
