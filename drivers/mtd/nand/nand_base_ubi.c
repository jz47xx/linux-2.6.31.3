...
#ifndef CONFIG_MTD_NAND_DMA
			//if (mtd->flags & MTD_NAND_CPU_MODE) { //if use mtd->flags, must modify partition_info[] in jz47xx_nand.c
				//printk("==>%s: sndcmd = %d, page = %d\n", __func__, sndcmd, page);
				if (likely(sndcmd)) {
					chip->cmdfunc(mtd, NAND_CMD_READ0, 0x00, page);
					sndcmd = 0;
				}
