...
//		 LCD_OSDC_F1EN | /* enable Foreground1 */
		 ...
		 .ipu_restart = 0x80001000, /* ipu restart */
		 .fg_change = FG_CHANGE_ALL, /* change all initially */
//		 .fg0 = {32, 0, 0, 720, 482}, /* bpp, x, y, w, h */
		 .fg0 = {32, 0, 0, 1440, 482}, /* bpp, x, y, w, h */
//		 .fg1 = {32, 0, 0, 720, 482}, /* bpp, x, y, w, h */
		 .fg1 = {32, 0, 0, 1440, 482}, /* bpp, x, y, w, h */
	 },
...
//		 LCD_OSDC_F1EN | /* enable Foreground1 */
	...
	.osd = {
		 ...
		 .osd_ctrl = 0,		/* disable ipu,  */
		 .rgb_ctrl = 0,
		 .bgcolor = 0xffffff, /* set background color Black */
		 .colorkey0 = 0, /* disable colorkey */
		 .colorkey1 = 0, /* disable colorkey */
		 .alpha = 0xA0,	/* alpha value */
		 .ipu_restart = 0x80001000, /* ipu restart */
...
struct jz4760lcd_info jz4760_info_hdmi_480p = {                            
		...
		640,480, 60, 96, 2,48,16,  33,10,       	//  width,height,freq,hsync,vsync,elw,blw,efw,bfw         
	},                                      
        .osd = {                                
                .osd_cfg =  LCD_OSDC_OSDEN      |               // Use OSD mode
                 LCD_OSDC_ALPHAEN               |               // enable alpha
...
struct jz4760lcd_info jz4760_info_hdmi_576p = {                            
        .panel = {                                                     
                ...
                .slcd_cfg = 0,                                            
		.ctrl = LCD_CTRL_BST_32,                                  
		720,576,50,64,5,68,12,40,4,                     //  width,height,freq,hsync,vsync,elw,blw,efw,bfw         
	},                                      
        .osd = {                                
                .osd_cfg =  LCD_OSDC_OSDEN      |               // Use OSD mode
                 LCD_OSDC_ALPHAEN               |               // enable alpha
...
struct jz4760lcd_info jz4760_info_hdmi_720p50 = {                            
        .panel = {                                                     
		       ...
		       LCD_CFG_PCP | LCD_CFG_HSP | LCD_CFG_VSP,           
                .slcd_cfg = 0,                                            
		.ctrl = LCD_CTRL_BST_32,                                  
		1280,720,50,40,5,440,220,20,5,                 //  width,height,freq,hsync,vsync,elw,blw,efw,bfw         
	},                                      
        .osd = {                                
                .osd_cfg =  LCD_OSDC_OSDEN      |               // Use OSD mode
		LCD_OSDC_ALPHAEN                |               // enable alpha
...
struct jz4760lcd_info jz4760_info_hdmi_720p60 = {                            
        .panel = {                                                     
                ...
                .slcd_cfg = 0,                                            
		.ctrl = LCD_CTRL_BST_32,                                  
		1280,720,60,40,5,110,220,20,5,	         	//  width,height,freq,hsync,vsync,elw,blw,efw,bfw         
	},                                      
        .osd = {                                
                .osd_cfg =  LCD_OSDC_OSDEN      |               // Use OSD mode
                 LCD_OSDC_ALPHAEN               |               // enable alpha
	...
	case FBIO_MODE_SWITCH:
		...
		switch (arg) {
			...
			case PANEL_MODE_TVE_NTSC: 	/* switch to TVE_NTSC mode */
				...
				screen_off();          /* turn off lcd backlight */
				break;
#endif
#if defined(CONFIG_JZ4760_HDMI_DISPLAY)  
...
//		while ( REG_LCD_OSDS & LCD_OSDS_READY )	/* fix in the future, Wolfgang, 06-20-2008 */
		...
		if ( lcd_info->osd.fg_change & FG0_CHANGE_SIZE ) { /* change FG0 size */
			if ( lcd_info->panel.cfg & LCD_CFG_TVEN ) { /* output to TV */
				...
				if((lcd_info->osd.fg0.h & 1) == 0)
				{	
					dma0_desc0->cmd = (fg0_frm_size/4) / 2;
					dma0_desc1->cmd = (fg0_frm_size/4) / 2;
					
				}else
				{
					dma0_desc0->cmd = (fg0_line_size * (lcd_info->osd.fg0.h / 2 + 1))/4;
					dma0_desc1->cmd = (fg0_line_size * (lcd_info->osd.fg0.h / 2))/4;
				}
				...
				if(lcd_info->osd.fg0.y % 2 ) {
					REG_LCD_DA0 = virt_to_phys(dma0_desc1); 
				}else{
		...
		if ( lcd_info->osd.fg_change & FG1_CHANGE_SIZE ) { /* change FG1 size*/
			if ( lcd_info->panel.cfg & LCD_CFG_TVEN ) { /* output to TV */
				...
				if((lcd_info->osd.fg1.h & 1) == 0)
				{	
					dma1_desc0->cmd = (fg1_frm_size/4) / 2;
					dma1_desc1->cmd = (fg1_frm_size/4) / 2;
					
				}else
				{
					dma1_desc0->cmd = (fg1_line_size * (lcd_info->osd.fg1.h / 2 + 1))/4;
					dma1_desc1->cmd = (fg1_line_size * (lcd_info->osd.fg1.h / 2))/4;
				}
...
static void jz4760fb_change_clock( struct jz4760lcd_info * lcd_info )
{
	...
	if ( lcd_info->panel.cfg & LCD_CFG_TVEN ) { 		/* LCDC output to TVE */
		...
		pclk = 27000000;
                ...
                /*get pixclk from pll0*/
		val = cpm_get_pllout() / pclk;   

                /*get pixclk from pll1*/
/*
		val = cpm_get_pllout1() / pclk;  
		REG_CPM_LPCDR |= LPCDR_LPCS;
*/
		val--;
