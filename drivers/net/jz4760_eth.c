...
static void dump_skb_data(struct sk_buff *skb) {
        int i = 0;
...
#define LINK_CHECK_INTERVAL (1 * HZ)

static void link_checker_handler(unsigned long data) {
	...
	mod_timer(&np->link_checker, jiffies + LINK_CHECK_INTERVAL);
}
...
static void start_check(struct net_device *dev) {
	...
	init_timer(&np->link_checker);
	np->link_checker.data = (unsigned long)dev;
	np->link_checker.function = link_checker_handler;
	np->link_checker.expires = jiffies + LINK_CHECK_INTERVAL;

	add_timer(&np->link_checker);
}
...
static int close_check(struct net_device *dev) {
	...
	del_timer_sync(&np->link_checker);
}
#else
static void start_check(struct net_device *dev)
{
	...
	t = kthread_run(link_check_thread,(void *)dev, dev->name);
	if (IS_ERR(t))
...
#endif	/* !use timer for link check */
