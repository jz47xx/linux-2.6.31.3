...
#define JZCODEC_RW_BUFFER_SIZE		8
...
static inline void audio_start_dma(int chan, void *dev_id, unsigned long phyaddr,int count, int mode)
{
	...
	release_dma_lock(flags);
#if 0
	__pcm_enable_tfs_intr(PCM_IN_USE);
	__pcm_enable_tur_intr(PCM_IN_USE);
	__pcm_enable_rfs_intr(PCM_IN_USE);
	__pcm_enable_ror_intr(PCM_IN_USE);
#endif
#endif
}
...
{
//	struct jz_pcm_controller_info *controller = (struct jz_pcm_controller_info *)dev_id;
	printk("pcm interrupt REG_PCM_PINTS : 0x%08x\n", REG_PCM_PINTS(PCM_IN_USE));
...
{
	__pcm_flush_fifo(PCM_IN_USE);
	__pcm_disable_txfifo(PCM_IN_USE);
	__pcm_disable_rxfifo(PCM_IN_USE);
	__pcm_set_transmit_trigger(PCM_IN_USE, 7);
	__pcm_set_receive_trigger(PCM_IN_USE, 7);
	__pcm_omsb_next_sync(PCM_IN_USE);
	__pcm_imsb_next_sync(PCM_IN_USE);
#if MODE_is_8
	__pcm_set_iss(PCM_IN_USE, 8);//8bits decided by LINSEL
	__pcm_set_oss(PCM_IN_USE, 8);//8bits decided by LINSEL
#else
	__pcm_set_iss(PCM_IN_USE, 16);//16bits decided by LINSEL
	__pcm_set_oss(PCM_IN_USE, 16);
#endif
	__pcm_disable_tfs_intr(PCM_IN_USE);
	__pcm_disable_tur_intr(PCM_IN_USE);
	__pcm_disable_rfs_intr(PCM_IN_USE);
	__pcm_disable_ror_intr(PCM_IN_USE);

	__pcm_disable_receive_dma(PCM_IN_USE);
	__pcm_disable_transmit_dma(PCM_IN_USE);
...
#if PCM_IN_USE == 0
	printk("DMA_ID_PCM0_TX=0x%08x DMA_ID_PCM0_RX=0x%08x\n",DMA_ID_PCM0_TX ,DMA_ID_PCM0_RX);
	if ((controller->dma1 = jz_request_dma(DMA_ID_PCM0_TX, "audio dac", jz_pcm_replay_dma_irq, IRQF_DISABLED, controller)) < 0) {
		printk(KERN_ERR "%s: can't reqeust DMA DAC channel.\n", name);
		goto dma1_failed;
	}
	if ((controller->dma2 = jz_request_dma(DMA_ID_PCM0_RX, "audio adc", jz_pcm_record_dma_irq, IRQF_DISABLED, controller)) < 0) {
		printk(KERN_ERR "%s: can't reqeust DMA ADC channel.\n", name);
		goto dma2_failed;
	}
#else
	printk("DMA_ID_PCM1_TX=0x%08x DMA_ID_PCM1_RX=0x%08x\n",DMA_ID_PCM1_TX ,DMA_ID_PCM1_RX);
	if ((controller->dma1 = jz_request_dma(DMA_ID_PCM1_TX, "audio dac", jz_pcm_replay_dma_irq, IRQF_DISABLED, controller)) < 0) {
		printk(KERN_ERR "%s: can't reqeust DMA DAC channel.\n", name);
		goto dma1_failed;
	}
	if ((controller->dma2 = jz_request_dma(DMA_ID_PCM1_RX, "audio adc", jz_pcm_record_dma_irq, IRQF_DISABLED, controller)) < 0) {
		printk(KERN_ERR "%s: can't reqeust DMA ADC channel.\n", name);
		goto dma2_failed;
	}
#endif
...
#else
#if PCM_IN_USE == 0
	err = request_irq(IRQ_PCM0, jz_pcm_irq, IRQF_DISABLED, "pcm irq", controller);
#else
	err = request_irq(IRQ_PCM1, jz_pcm_irq, IRQF_DISABLED, "pcm irq", controller);
#endif
#endif
	if (err < 0)
...
static void __exit unload_jz_pcm(struct jz_pcm_controller_info *controller)
{
	...
	__pcm_reset(PCM_IN_USE);
	schedule_timeout(5);
	__pcm_disable(PCM_IN_USE);
	__pcm_clk_disable(PCM_IN_USE);
	__pcm_flush_fifo(PCM_IN_USE);
...
static int __init init_jz_pcm(void)
{
...
#ifndef CONFIG_SOC_JZ4770
	/* reset codec GPB30 */
	__gpio_as_output(32 * 1 + 30);
	__gpio_set_pin(32 * 1 + 30);
#else
	/* reset codec GPF22 */
	__gpio_as_output0(GPF(22));
	mdelay(10);
	__gpio_as_output1(GPF(22));
#endif

#if 0
	__gpio_as_func0(32 * 3 + 0);
	...
	printk("====>test gpio\n");
	while(1) {
		__gpio_as_output0(GPF(12));
		__gpio_as_output0(GPF(13));
		__gpio_as_output0(GPF(14));
		__gpio_as_output0(GPF(15));
		mdelay(10);
		__gpio_as_output1(GPF(12));
		__gpio_as_output1(GPF(13));
		__gpio_as_output1(GPF(14));
		__gpio_as_output1(GPF(15));
		mdelay(10);
	}
#endif
	__gpio_as_pcm1();
...
#ifndef CONFIG_SOC_JZ4770
	cpm_start_clock(CGM_PCM);
#else
#if PCM_IN_USE == 0
	cpm_start_clock(CGM_PCM0);
#else
	cpm_start_clock(CGM_PCM1);
#endif
#endif
	__pcm_enable(PCM_IN_USE);
	//__pcm_clk_enable(PCM_IN_USE);
	...
	cpm_set_clock(CGU_PCMCLK, 129000000);

	printk("===>pcm_sys_clk = %d\n", cpm_get_clock(CGU_PCMCLK));
	...
	//__pcm_set_clk_rate(PCM_IN_USE, cpm_get_clock(CGU_PCMCLK), 2048000);
	__pcm_set_clk_div(PCM_IN_USE, 62);

	printk("===>pcmclk div = %d\n", ((REG_PCM_PDIV(PCM_IN_USE) & PDIV_CLKDIV_MASK) >> PDIV_CLKDIV_LSB));
	printk("===>PCM_PDIV(n) = 0x%08x\n", PCM_PDIV(PCM_IN_USE));
	// ori:
	...
	// ori:
	__pcm_set_sync_rate(PCM_IN_USE, 2048000,8000);

	//__pcm_set_sync_rate(2000000, 8000);
	__pcm_set_sync_len(PCM_IN_USE, 0);
	...
	//printk("^^^^^^^^^^^^^^^^first the jz_audio_channels is %d\n",jz_audio_channels);

#if MODE_is_8
	__pcm_set_iss(PCM_IN_USE, 8);//8bits decided by LINSEL
	__pcm_set_oss(PCM_IN_USE, 8);
#else
	__pcm_set_iss(PCM_IN_USE, 16);//16bits decided by LINSEL
	__pcm_set_oss(PCM_IN_USE, 16);
#endif
	__pcm_set_valid_slot(PCM_IN_USE, 1);
	...
	__pcm_last_sample(PCM_IN_USE);
	__pcm_as_master(PCM_IN_USE);
	...
	jz_update_filler(AFMT_S16_LE, 2);
	//jz_update_filler(AFMT_U8, 1);
...
static int jz_audio_release(struct inode *inode, struct file *file)
{
	...
	if ( controller->opened1 == 1  ) {
		__pcm_enable_transmit_dma(PCM_IN_USE);
		__pcm_enable_txfifo(PCM_IN_USE);
	...
	if ( controller->opened2 == 1  ) {
		first_record_call = 1;
		__pcm_enable_receive_dma(PCM_IN_USE);
		__pcm_enable_rxfifo(PCM_IN_USE);
...
static int jz_audio_open(struct inode *inode, struct file *file)
{
	...
	if (file->f_mode & FMODE_WRITE) {
		...
		/* set PCMOUT params */
#if 0
		jz_audio_set_format(controller->dev_audio, 16);
 ...
 */
int jz_pcm_direct_write(const char *buffer, size_t count);
int jz_pcm_direct_read(char *buffer, size_t count);

EXPORT_SYMBOL(jz_pcm_direct_write);
EXPORT_SYMBOL(jz_pcm_direct_read);
...
audio_read_back_first:
		...
		if ((id = get_buffer_id(&in_empty_queue)) >= 0) {
			...
			*(in_dma_buf_data_count + id) = copy_count;
			spin_unlock(&controller->lock);
			dma_cache_wback_inv(*(in_dma_buf + id), *(in_dma_buf_data_count + id));
			audio_start_dma(controller->dma2,
					pcm_controller,
					*(in_dma_pbuf + id),
					*(in_dma_buf_data_count + id),
					DMA_MODE_READ);
...
int jz_pcm_direct_write(const char *buffer, size_t count)
{
	...
	memcpy(controller->tmp1, buffer, count);
