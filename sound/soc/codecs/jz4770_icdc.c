			    ...
			    struct snd_soc_dai *dai)
{
	...
	if (playback) {
		...
		case 1:
			...
			jz_icdc_update_reg(codec, JZ_ICDC_CR_DAC, 6, 0x1, 1);
			break;
		case 2:
			jz_icdc_update_reg(codec, JZ_ICDC_CR_DAC, 6, 0x1, 0);
			break;
		}
	}
...
static struct snd_soc_codec *jz4770_codec;
static struct jz_icdc_priv jz4770_priv;
...
static const char *jz_icdc_hp_sel[] = {"Mic 1b", "Mic 2b", "Line Inb", "Stereo DAC"};
static const char *jz_icdc_lineout_sel[] = {"Mic 1_lo", "Mic 2_lo", "Line In_lo", "Stereo DAC"};
...
static const struct snd_soc_dapm_widget jz_icdc_dapm_widgets[] = {
	...
	SND_SOC_DAPM_MICBIAS_E("Mic Bias", JZ_ICDC_CR_MIC, 0, 1,
			       micbias_event,
...
static int jz_icdc_probe(struct platform_device *pdev)
{
	...
	if (jz4770_codec == NULL) {
		dev_err(&pdev->dev, "Codec device not registered\n");
		return -ENODEV;
	}

	socdev->card->codec = jz4770_codec;
	codec = jz4770_codec;
...
{
	int ret, i;
	struct snd_soc_codec *codec = &jz4770_priv.codec;
	u8 *cache = NULL;
	...
	memset(&jz4770_priv, 0, sizeof(jz4770_priv));
	...
	codec->reg_cache = &jz4770_priv.reg_cache;
	codec->private_data = &jz4770_priv;
...
{
	/* clr SB_SLEEP */
	jz_icdc_update_reg(&jz4770_priv.codec, JZ_ICDC_CR_VIC, 1, 0x1, 1);
	mdelay(10);

	/* clr SB */
	jz_icdc_update_reg(&jz4770_priv.codec, JZ_ICDC_CR_VIC, 0, 0x1, 1);

	jz_icdc_set_bias_level(&jz4770_priv.codec, SND_SOC_BIAS_OFF);
	snd_soc_unregister_dai(&jz_icdc_dai);
	snd_soc_unregister_codec(&jz4770_priv.codec);
	jz4770_codec = NULL;
}
